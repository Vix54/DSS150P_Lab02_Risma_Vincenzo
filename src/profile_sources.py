...
def profile_json(path):
    ...
    """Profile a JSON source: record count, keys, nested fields, timestamp/numeric fields, nulls/missing keys."""
    with open(path) as f:
        data = json.load(f)
    ...
    # Union of all keys seen across all records — JSON has no fixed schema per record
    all_keys = set()
    for record in data:
